...
`GTANetwork.dll` is **.NET Framework 4.8** (loaded into `GTA5.exe` by a C++/CLI ScriptHookVDotNet fork, in a
second AppDomain). The browser is **CefSharp / Chromium 151 in a separate process** (`cef\GTANetwork.CefHost.exe`,
`Subprocess/GTANetwork.CefHost`; protocol and shared-memory frames in `Shared/Cef`), client scripting on
**ClearScript 7.5 (V8 12)**.
...
eng/dev-build-client.sh --sync                  # dev container: rebuild client + browser host into ~/GTANetwork (~10 s)
eng/dev-test.sh                                 # the Linux CI checks locally (server smoke + bot integration)
eng/cef-harness.sh [--install-cef]              # on the host machine: the browser process under Proton, no game needed
```

The dev container (`.devcontainer/`, `docker-compose.yml`) exists so client changes can be tried in game
without waiting for CI. Only the C++/CLI `ScriptHookVDotNet.dll` needs Windows + MSVC; a client meant for the
game must be compiled against that real DLL (`eng/dev-build-client.sh` takes it from the install), never against
the `Shv.NET/ref` stub, which is compile-check only. See `docs/DEVCONTAINER.md`.
