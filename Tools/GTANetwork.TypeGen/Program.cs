...
internal static class Program
{
    ...
    {
        string? client = null, server = null, net48Refs = null, outDir = "types", runtimeLib = null;
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            switch (args[i])
            {
                ...
                case "--runtime-lib": runtimeLib = Next(); break; // folder of the Bun runtime library: writes api.generated.d.ts there
        ...
        if (server != null)
        {
            ...
            if (runtimeLib != null)
            {
                ...
                var lib = RuntimeApiEmitter.Build(api, emitter.Docs);
                File.WriteAllText(Path.Combine(runtimeLib, "api.generated.d.ts"), lib.text, new UTF8Encoding(false));
                var enums = RuntimeApiEmitter.BuildEnums();
                File.WriteAllText(Path.Combine(runtimeLib, "enums.generated.ts"), enums.text, new UTF8Encoding(false));
                report.AppendLine($"runtime library: {lib.members} members in {Path.Combine(runtimeLib, "api.generated.d.ts")}, {enums.count} enums in enums.generated.ts");
            }
        }
...
""",
    ...
    public string Render(string source, Version? version, string header)
    {
        ...
        sb.AppendLine("// Generated by Tools/GTANetwork.TypeGen from " + source + ". Do not edit; run the generator (types/README.md).");
...
internal static class Catalogue
{
    public static string Build(Type api, Emitter emitter)
    {
        ...
        foreach (var m in api.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Where(m => !m.IsSpecialName && !m.Name.Contains('<')))
        {
            ...
            {
                name = m.Name,
                parameters = m.GetParameters().Select(p => new { name = p.Name, type = StableName(p.ParameterType), ts = emitter.Map(p.ParameterType), optional = p.IsOptional || p.HasDefaultValue, isParams = p.ParameterType.IsArray && p.GetCustomAttributesData().Any(a => a.AttributeType.FullName == "System.ParamArrayAttribute") }).ToArray(),
                returns = StableName(m.ReturnType),
        ...
        var events = api.GetEvents(BindingFlags.Public | BindingFlags.Instance).Select(e =>
        {
            ...
            return new { name = e.Name, parameters = invoke?.GetParameters().Select(p => new { name = p.Name, type = StableName(p.ParameterType), ts = emitter.Map(p.ParameterType) }).ToArray() };
        }).ToArray();
        // no assembly version: it is time-based (0.1.<days>.<minutes/2>) and would make every CI build report stale typings
        var doc = new { generatedBy = "Tools/GTANetwork.TypeGen", assembly = api.Assembly.GetName().Name, functions, events };
        return JsonSerializer.Serialize(doc, new JsonSerializerOptions { WriteIndented = true }).Replace("\r\n", "\n") + "\n";
    }

    /// <summary>A type name that does not change between builds: namespace-qualified, generic arguments recursively, no assembly names.</summary>
    private static string StableName(Type t)
    {
        if (t.IsByRef) return StableName(t.GetElementType()!) + "&";
        if (t.IsArray) return StableName(t.GetElementType()!) + "[]";
        if (t.IsGenericParameter) return t.Name;
        if (t.IsGenericType)
        {
            var def = t.GetGenericTypeDefinition();
            var baseName = (def.Namespace != null ? def.Namespace + "." : "") + def.Name;
            return baseName + "[" + string.Join(",", t.GetGenericArguments().Select(StableName)) + "]";
        }
        return (t.Namespace != null ? t.Namespace + "." : "") + t.Name;
    }
}

/// <summary>
/// The typed shape of <c>gtan.api</c> for TypeScript server resources (runtime/gtan): every API member as a method that returns
/// a Promise (the bridge is asynchronous), players and entities as handles (numbers), Vector3 as {x, y, z}, enums as numbers.
/// </summary>
internal static class RuntimeApiEmitter
{
    /// <summary>Enums the API's signatures mention; emitted as runtime tables so TypeScript resources can parse names (`/veh adder`).</summary>
    private static readonly Dictionary<string, Type> SeenEnums = new(StringComparer.Ordinal);

    /// <summary>gtan.enums: `export const VehicleHash = Object.freeze({ Adder: -1216765807, ... })` for every enum the API uses.</summary>
    public static (string text, int count) BuildEnums()
    {
        var sb = new StringBuilder();
        sb.AppendLine("// Generated by Tools/GTANetwork.TypeGen from GTANetworkServer.dll: the enums the server API uses, as runtime tables");
        sb.AppendLine("// (gtan.enums.VehicleHash.Adder, gtan.parseEnum(gtan.enums.VehicleHash, \"adder\")). Do not edit; run the generator (types/README.md).");
        sb.AppendLine("export type EnumTable = Readonly<Record<string, number>>;");
        var count = 0;
        foreach (var (name, t) in SeenEnums.OrderBy(kv => kv.Key, StringComparer.Ordinal))
        {
            sb.AppendLine();
            sb.AppendLine("export const " + name + " = Object.freeze({");
            var seenMembers = new HashSet<string>(StringComparer.Ordinal);
            foreach (var f in t.GetFields(BindingFlags.Public | BindingFlags.Static))
            {
                if (!seenMembers.Add(f.Name)) continue;
                var raw = f.GetRawConstantValue();
                var value = raw == null ? "0" : Convert.ToString(raw, CultureInfo.InvariantCulture);
                sb.AppendLine("    " + Ident(f.Name) + ": " + value + ",");
            }
            sb.AppendLine("});");
            count++;
        }
    ...
    public static (string text, int members) Build(Type api, XmlDocs docs)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// Generated by Tools/GTANetwork.TypeGen from GTANetworkServer.dll: the server API as the Bun runtime exposes it (gtan.api).");
        sb.AppendLine("// Do not edit; run the generator (types/README.md). Players and entities are handles; every call returns a Promise.");
        sb.AppendLine("export type Handle = number;");
        sb.AppendLine("export interface Vec3 { x: number; y: number; z: number; }");
        sb.AppendLine("export interface Rgba { r: number; g: number; b: number; a?: number; }");
        sb.AppendLine();
        sb.AppendLine("export interface ServerApi {");
        var members = 0;
        var seen = new HashSet<string>(StringComparer.Ordinal);
        foreach (var m in api.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly).Where(m => !m.IsSpecialName && !m.Name.Contains('<')).OrderBy(m => m.Name, StringComparer.Ordinal))
        {
            string line;
            try
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var p = ps[i];
                    var isParams = i == ps.Length - 1 && p.ParameterType.IsArray && p.GetCustomAttributesData().Any(a => a.AttributeType.FullName == "System.ParamArrayAttribute");
                    var name = Ident(p.Name ?? ("arg" + i));
                    parts.Add(isParams ? "..." + name + ": " + Map(p.ParameterType) : name + (p.IsOptional || p.HasDefaultValue ? "?" : "") + ": " + Map(p.ParameterType));
                }
                var ret = m.ReturnType.FullName == "System.Void" ? "void" : Map(m.ReturnType);
                line = "    " + Ident(m.Name) + "(" + string.Join(", ", parts) + "): Promise<" + ret + ">;";
            }
            catch (Exception ex) when (ex is FileNotFoundException or TypeLoadException)
            {
                line = "    // " + m.Name + ": not emitted (" + ex.GetType().Name + ")";
            }
    ...
    {
        if (t.IsByRef) return Map(t.GetElementType()!);
        if (t.IsArray) return Wrap(Map(t.GetElementType()!)) + "[]";
        var full = t.IsGenericType ? t.GetGenericTypeDefinition().FullName : t.FullName;
        switch (full)
        {
            ...
            case "System.Collections.Generic.List`1": case "System.Collections.Generic.IEnumerable`1": case "System.Collections.Generic.IList`1":
                return Wrap(Map(t.GetGenericArguments()[0])) + "[]";
            case "System.Collections.Generic.Dictionary`2": return "Record<string, " + Map(t.GetGenericArguments()[1]) + ">";
            case "GTANetworkShared.Vector3": return "Vec3";
            case "GTANetworkShared.NetHandle": return "Handle";
            case "GTANetworkServer.Constant.Color": return "Rgba";
            case "GTANetworkServer.XmlGroup": return "string";
            case "GTANetworkServer.ColShape": return "Handle";
        }
        if (t.IsEnum)
        {
            SeenEnums.TryAdd(t.Name, t);
            return "number /* " + t.Name + " */";
        }
        if (t.FullName == "GTANetworkServer.Client" || (t.Namespace == "GTANetworkServer" && IsEntity(t))) return "Handle /* " + t.Name + " */";
        if (t.BaseType?.FullName == "System.MulticastDelegate") return "never /* callback: use events */";
        return "unknown /* " + full + " */";
    }

    private static bool IsEntity(Type t)
    {
        for (var b = t; b != null; b = b.BaseType) if (b.FullName == "GTANetworkServer.Entity") return true;
        return false;
    }

    private static string Wrap(string ts) => ts.Contains(" | ") || ts.Contains(" => ") ? "(" + ts + ")" : ts;

    private static readonly HashSet<string> Reserved = new(StringComparer.Ordinal) { "delete", "default", "function", "class", "new", "var", "in", "of", "arguments", "eval", "typeof", "void", "with", "yield", "let", "static", "enum", "export", "import", "super", "this", "null", "true", "false", "return", "switch", "case", "break", "continue", "do", "while", "for", "if", "else", "try", "catch", "finally", "throw", "instanceof", "debugger", "const", "extends", "implements", "interface", "package", "private", "protected", "public", "await" };
    private static string Ident(string name) => Reserved.Contains(name) ? name + "_" : name;
}
