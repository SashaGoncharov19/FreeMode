...
| Sync, server performance | bot load runs (`Tools/GTANetwork.Bot`, N instances against one server) — see the load-test task in `docs/tasks/` | dev container / host | Tick time, bytes/s per player, packet loss at N players. |
