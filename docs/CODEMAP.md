...
| `Managers/Streamer.cs` | Interest management (T-003): every 250 ms a grid of `cell`-metre cells per dimension; per sender the tiers `Full` (≤ 50 m, ≤ maxfull, every pure packet), `Medium` (≤ 200 m, every 3rd), `Low` (≤ range, ≤ maxnear together, every 10th), `Far` (everyone else, a position every 3 s); dimension 0 sees and is seen by all. `Packets.cs` `CollectNear/CollectFar/AddTier` pick the tiers by the sender's `SyncSequence` and apply the recipient's byte budget (`InterestSettings`). |
| `Managers/NetEntityHandler.cs` | Server entity registry, handles, `UpdateMovements`, `CreateWorld`; creates go through `GameServer.PublishEntity` (T-026: the players in range; blips/world/players to everyone), deletes to the players who know the entity. |
| `Managers/FileServer.cs` | `HttpListener` on the game port (TCP): `GET /manifest.json`, `GET /<resource>/<path>` for declared `<file>`s; traversal guard; `GET /metrics.json` (T-002); `GET /dlcpacks.json` (T-014). |
...
| `Sync/SyncPed.cs` (491), `Sync/Entity/OnFoot.cs` (1034), `Entity/Vehicle.cs` (513), `Entity/UnoccupiedVehicle.cs`, `Sync/Interpolation.cs`, `Sync/Misc.cs`, `Sync/Nametag.cs`, `Sync/SyncEventWatcher.cs`, `Sync/SyncSender/*` | Remote player representation and rendering (coords forcing, tasks, interpolation, ragdoll/parachute/cover), sync event detection (doors, lights, trailers, tyres), local data collection. Review and open findings: `docs/SYNC.md`. `Sync/SyncMetrics.cs` (T-018): the per-player render error / packet age / rate, the 10-second `[SYNC]` summary, `RouteRecorder` (`GTAN_RECORD_ROUTE=1` → `logs/route-*.jsonl`). |
...
| `Util/AutoConnect.cs` | `GTAN_CONNECT=host:port[#key][;password]` from the launcher (T-024): joins that server as soon as the game is ready, no menu. |
...
| File | Content |
| --- | --- |
| `Launcher/Program.cs` | The command line front end: commands (run/deploy/restore/doctor/`prepare <host:port>` for a server's DLC packs), options, `--debug`, `--connect host:port[#key]` (T-024); calls `Launcher.Core`. |
| `Launcher.Core/LaunchSession.cs`, `Deployment.cs`, `DlcPacks.cs`, `ServerList.cs`, `GamePatcher.cs`, `Steam.cs`, `Vdf.cs`, `Paths.cs`, `GameProcess.cs`, `Log.cs`, `HitchMonitor.cs` | The launcher's logic as a library: `DetectedEnvironment.Detect`, `SettingsStore`, `LaunchSession.Play/Launch/Doctor` (steam/proton/direct, wait for `GTA5.exe`, restore), deploy/restore of mod files, `DlcPacks.Fetch/Download/Prepare` (a server's `/dlcpacks.json`, SHA-256-verified downloads into `dlcpacks/<name>/`, T-014), `ServerList.Fetch/Merge` (the master's `/servers/full` + favourites/recent, T-024), GTA V settings patching, Steam library/Proton/prefix detection, install paths, process lookup by `/proc`, `Log` with a `Written` event, the `--debug` system monitor. |
| `Launcher.Gui/` (`Program.cs`, `App.axaml`, `Views/MainWindow.axaml`, `ViewModels/MainViewModel.cs`) | The launcher window (Avalonia 12, CommunityToolkit.Mvvm): Home (doctor lines, Play/Stop, the log), Servers (the master list, favourites, recent, direct connect → `GTAN_CONNECT`, T-024), Packs (T-014), Settings (`PlayerSettings` ↔ `settings.xml`), Logs (`logs/*.log` tail). `--self-test` runs it headless (`Avalonia.Headless`). Installed into `<install>/gui/`. |
| `Tools/GTANetwork.Bot/Program.cs`, `LoadBots.cs` | Options (`--host`, `--port`, `--name`, `--password`, `--say`, `--expect`, `--rpc`, `--pin`, `--duration`, `--no-sync`, `--discover`, `--download-files`, `-i`), one Lidgren connection, the protocol handshake and sync loop, chat assertions. `--bots N --move <m> --report <file>` (T-002, `LoadBots.cs`): N connections in one process on a few pump threads, each joining like a client and sending pure/light sync at the client's rates while random-walking. Also compiles `Shv.NET/ref/Core/NativeHashes.g.cs`. |
...
`integration-test-auth.sh`, `integration-test-template.sh`, `integration-test-master.sh`, `load-test.sh` (T-002: `<players> <seconds>`, samples `/metrics.json`, writes `artifacts/load-<N>.json`, prints the baseline table; `LOAD_PLAYERS=N eng/dev-test.sh` runs it after the checks), `dev-test.sh`, `dev-build-client.sh`, `dev-sync-client.sh`, `package-client.ps1`,
