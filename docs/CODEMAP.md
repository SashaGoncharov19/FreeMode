...
| File | Content |
| --- | --- |
| `Launcher/Program.cs` | The command line front end: commands (run/deploy/restore/doctor/`prepare <host:port>` for a server's DLC packs), options, `--debug`; calls `Launcher.Core`. |
| `Launcher.Core/LaunchSession.cs`, `Deployment.cs`, `DlcPacks.cs`, `GamePatcher.cs`, `Steam.cs`, `Vdf.cs`, `Paths.cs`, `GameProcess.cs`, `Log.cs`, `HitchMonitor.cs` | The launcher's logic as a library: `DetectedEnvironment.Detect`, `SettingsStore`, `LaunchSession.Play/Launch/Doctor` (steam/proton/direct, wait for `GTA5.exe`, restore), deploy/restore of mod files, `DlcPacks.Fetch/Download/Prepare` (a server's `/dlcpacks.json`, SHA-256-verified downloads into `dlcpacks/<name>/`, T-014), GTA V settings patching, Steam library/Proton/prefix detection, install paths, process lookup by `/proc`, `Log` with a `Written` event, the `--debug` system monitor. |
