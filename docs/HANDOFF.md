...
* **In PR**: T-009 encrypted sessions (`task/T-009-session-crypto`, D-14): X25519 + HKDF in the hail, AES-256-GCM per message,
  `server.key`, `RequireEncryption` default true (old clients are refused — client and server must be updated together), pin with
  `host:port#key`. After the merge the deploy must sync the client and the server at the same time.
...
0. **Tasks, in order** (`docs/PLAN.md` §4, D-12): merge T-009 when green → **T-010** launcher GUI → **T-011**
