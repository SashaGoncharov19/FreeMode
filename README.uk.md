...
* **Розходження версій гри.** ScriptHookV має відповідати встановленому білду GTA V, а патерни пам'яті /
  хеші нативів у `Shv.NET/source/core` та `Client/` — з 2019–2020 років. Нові Legacy-білди можуть вимагати
  оновлених офсетів; GTA V Enhanced не підтримується.
* **У грі нічого не тестувалося під час відродження** — у CI немає GTA V. Збірка, сервер і лаунчер покриті
  тестами; сам хук компілюється, але не перевірений.
* **ScriptHookV не можна розповсюджувати** — кожен користувач завантажує його сам.
* **Мастер-сервера немає**: без публічного списку серверів і автооновлень, `announce` вимкнено.
