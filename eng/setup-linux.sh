...
# Page-align the browser's PE files so that Wine maps them from disk (shared between Chromium's processes) instead
# of copying each into every process; packages built after 0.2.0-alpha.6 arrive aligned, this covers older ones.
python3 - "$DIR/cef" <<'PY' 2>/dev/null || true
#!/usr/bin/env python3
